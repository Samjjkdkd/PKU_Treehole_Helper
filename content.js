...
class TreeholeHelper {
    constructor(){
        ...
        this.displayHolesFunction = null;
    ...
    createFloatingPanel() {
        ...
        this.registerDisplayHolesFunction(displayHoles);
        ...
        endTimeInput.addEventListener('change', () => {
            if (this.timeReachLimited) {
                this.timeReachLimited = false;
                console.log("[PKU TreeHole] 时间限制已修改，重置标记");
                this.updateGlobalStatus('时间限制已修改，可以重新开始收集数据');
            }
        });
        
        // 监听帖子数量限制输入框变化
        postsLimitInput.addEventListener('change', () => {
            if (this.postsReachLimited) {
                this.postsReachLimited = false;
                console.log("[PKU TreeHole] 帖子数量限制已修改，重置标记");
                this.updateGlobalStatus('帖子数量限制已修改，可以重新开始收集数据');
            }
        });
        ...
        clearTimeBtn.addEventListener('click', () => {
            ...
            if (this.timeReachLimited) {
                ...
                this.updateGlobalStatus('时间限制已清除，可以重新开始收集数据');
            }
        ...
        startBtn.addEventListener('click', () => {
            // 检查是否已经达到了发布时间限制
            if (this.timeReachLimited) {
                this.updateGlobalStatus('已达到发布时间限制，请修改或清除时间限制后再试', true);
            ...
            // 检查是否已经达到了帖子数量限制
            if (this.postsReachLimited) {
                this.updateGlobalStatus('已达到帖子数量限制，请增加数量限制后再试', true);
                return;
            }
                    ...
                    if (headerElement) {
                        ...
                        if (timeMatch) {
                            ...
                            if (endTimes > postTime) {
                                this.updateGlobalStatus('错误：设定的截止时间晚于当前可见帖子的发布时间', true);
            ...
            try {
                ...
                this.updateGlobalStatus(`开始收集数据，当前已有 ${currentCount || 0} 条数据${autoScrollEnabled ? '' : '（手动滚动模式）'}`);
            } catch (error) {
                this.updateGlobalStatus('收集数据失败: ' + error.message, true);
                this.stopCollection(true, '出现错误');
            }
        });

        stopBtn.addEventListener('click', () =>{
            this.stopCollection(true, '');//手动停止收集不提示
        });

        exportTextBtn.addEventListener('click', () => {
            this.exportHolesAsText();
        });

        exportImageBtn.addEventListener('click', () => {
            this.exportHolesAsImage();
        });

        // 添加排序方式变更监听
        panel.querySelector('#sort-method').addEventListener('change', () => {
            displayHoles(this.holesData);
        });
        ...
        setInterval(() => {
            if (this.isCollecting) {
                ...
                this.updateGlobalStatus(`已收集 ${this.holesData.length} 条数据，用时 ${elapsedTime.toFixed(0)} 秒${lastTime ? '，最后帖子发布于 ' + lastTime : ''}`);
            }
        }, 1000);
        ...
        batchClassifyBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // 开始分类
                this.startClassifying(apiSettings.apiKey);
        ...
        const startClassifying = (apiKey) => {
            ...
            const processNextHole = async () => {
                ...
                if (holeElement) {
                    ...
                    try {
                        // 执行分类 (这里会等待API响应)
                        category = await this.classifyTreehole(hole.content, apiKey);
        ...
        // 停止分类的函数
        const stopClassifying = (completed = false) => {
    ...
    registerDisplayHolesFunction(func) {
        console.log("[DEBUG] registerDisplayHolesFunction 被调用");
        this.displayHolesFunction = func;
    }

    // 全局调用 displayHoles 的包装函数
    displayHolesGlobal(holes) {
        console.log("[DEBUG] displayHolesGlobal 被调用");
        if (this.displayHolesFunction) {
            this.displayHolesFunction(holes);
        } else {
        ...
        if (updateUI) {
            ...
            if (panel) {
                ...
                try {
                    ...
                    // 使用延时来避免可能的递归调用
                    setTimeout(() => {
                        this.displayHolesGlobal(this.holesData);
                    }, 0);
                } catch (error) {
...
let PKUTreeHole = new TreeholeHelper();
PKUTreeHole.start();
